...
gamescope.config.known_displays[panel_id] = {
  ...
  dynamic_modegen = function(base_mode, refresh)
    debug("Generating mode "..refresh.."Hz for TF Handle 5 LCD")
  ...
  matches = function(display)
    local lcd_types = {
      { vendor = "LEN", model = "Go Display", product = 0x0001, name = "TF Handle 5 LCD" },
    }
    ...
    for index, value in ipairs(lcd_types) do
      if value.vendor == display.vendor and value.model == display.model and value.product == display.product then
        debug("[tf_handle5_lcd] Matched vendor: "..display.vendor.." model: "..display.model.." product: "..display.product)
        return 9900
      end
    end
    return -1
...
debug("Registered TF Handle 5 LCD as a known display")
debug(inspect(gamescope.config.known_displays.tf_handle5_lcd))